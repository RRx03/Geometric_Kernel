#include <metal_stdlib>
using namespace metal;

#include "../SDFShared.h"

// Enum constants for Metal
constant int C_DC=-1, C_SPHERE=0, C_BOX=1, C_CYL=2, C_TORUS=3, C_CAPSULE=4;
constant int C_CIRCLE2D=10, C_RECT2D=11, C_BEZ2D=12, C_CBEZ2D=13, C_CSPLINE2D=14;
constant int C_UNION=100, C_SUB=101, C_INTER=102, C_SMOOTH=103, C_XFORM=200;

// Helpers to read float3/float2 from flat struct
inline float3 ndPos(SDFNodeGPU n) { return float3(n.pos_x, n.pos_y, n.pos_z); }
inline float2 ndPos2(SDFNodeGPU n) { return float2(n.pos_x, n.pos_y); }
inline float4 ndPar(SDFNodeGPU n) { return float4(n.param_x, n.param_y, n.param_z, n.param_w); }

struct RasterizerData { float4 position [[position]]; float2 uv; };

// ── Math utilities ──
float cross2D(float2 a, float2 b) { return a.x*b.y - a.y*b.x; }

float2 evalQuad(float2 A, float2 B, float2 C, float t) {
    float u=1.0-t; return u*u*A + 2.0*u*t*B + t*t*C;
}
float2 evalQuadD(float2 A, float2 B, float2 C, float t) {
    return 2.0*((1.0-t)*(B-A)+t*(C-B));
}
float2 evalQuadD2(float2 A, float2 B, float2 C) { return 2.0*(A-2.0*B+C); }

float2 evalCubic(float2 P0, float2 P1, float2 P2, float2 P3, float t) {
    float u=1.0-t; return u*u*u*P0 + 3.0*u*u*t*P1 + 3.0*u*t*t*P2 + t*t*t*P3;
}
float2 evalCubicD(float2 P0, float2 P1, float2 P2, float2 P3, float t) {
    float u=1.0-t; return 3.0*(u*u*(P1-P0)+2.0*u*t*(P2-P1)+t*t*(P3-P2));
}
float2 evalCubicD2(float2 P0, float2 P1, float2 P2, float2 P3, float t) {
    return 6.0*((1.0-t)*(P2-2.0*P1+P0)+t*(P3-2.0*P2+P1));
}

// ── Cubic solver (Cardano) ──
int solveCubic(float a, float b, float c, float d, thread float r[3]) {
    const float E=1e-10, TP=6.283185307;
    if(abs(a)<E){if(abs(b)<E){if(abs(c)<E)return 0;r[0]=-d/c;return 1;}
        float D=c*c-4.0*b*d;if(D<0)return 0;float sq=sqrt(D);
        r[0]=(-c+sq)/(2.0*b);r[1]=(-c-sq)/(2.0*b);return D<E?1:2;}
    float p=(3.0*a*c-b*b)/(3.0*a*a);
    float q=(2.0*b*b*b-9.0*a*b*c+27.0*a*a*d)/(27.0*a*a*a);
    float off=-b/(3.0*a), D=-(4.0*p*p*p+27.0*q*q);
    if(D>E){float m=2.0*sqrt(-p/3.0);float arg=clamp(3.0*q/(p*m),-1.0,1.0);
        float th=acos(arg)/3.0;r[0]=m*cos(th)+off;r[1]=m*cos(th-TP/3.0)+off;
        r[2]=m*cos(th-2.0*TP/3.0)+off;return 3;
    }else{float sq=sqrt(max(0.0,q*q/4.0+p*p*p/27.0));
        float v1=-q/2.0+sq,v2=-q/2.0-sq;
        float u=sign(v1)*pow(abs(v1),1.0/3.0),v=sign(v2)*pow(abs(v2),1.0/3.0);
        r[0]=u+v+off;return 1;}
}

// ── Newton for quadratic Bézier (3 iters) ──
float newtonQ(float2 p, float2 A, float2 B, float2 C, float t0) {
    float t=t0; float2 d2=evalQuadD2(A,B,C);
    for(int i=0;i<3;i++){float2 diff=evalQuad(A,B,C,t)-p;float2 dt=evalQuadD(A,B,C,t);
        float f=dot(diff,dt),fp=dot(dt,dt)+dot(diff,d2);
        if(abs(fp)>1e-10)t=clamp(t-f/fp,0.0,1.0);}return t;
}

// ── Hybrid distance to quadratic Bézier ──
float distQBez(float2 p, float2 A, float2 B, float2 C) {
    float2 u=B-A,v=A-2.0*B+C,w=A-p;
    float ca=2.0*dot(v,v),cb=6.0*dot(u,v),cc=2.0*(2.0*dot(u,u)+dot(w,v)),cd=2.0*dot(w,u);
    float roots[3]; int nr=solveCubic(ca,cb,cc,cd,roots);
    float cands[5]; int nc=0; cands[nc++]=0.0; cands[nc++]=1.0;
    for(int i=0;i<nr;i++)if(roots[i]>=-0.01&&roots[i]<=1.01)cands[nc++]=clamp(roots[i],0.0,1.0);
    float best=1e10;
    for(int i=0;i<nc;i++){float t=newtonQ(p,A,B,C,cands[i]);best=min(best,length(p-evalQuad(A,B,C,t)));}
    return best;
}

// ── Cubic Bézier distance (sample + Newton) ──
float distCBez(float2 p, float2 P0, float2 P1, float2 P2, float2 P3) {
    float bt=0,bd=1e10;
    for(int j=0;j<=8;j++){float t=float(j)/8.0;float d=length(p-evalCubic(P0,P1,P2,P3,t));
        if(d<bd){bd=d;bt=t;}}
    float t=bt;
    for(int i=0;i<4;i++){float2 diff=evalCubic(P0,P1,P2,P3,t)-p;
        float2 dt=evalCubicD(P0,P1,P2,P3,t);float2 d2=evalCubicD2(P0,P1,P2,P3,t);
        float f=dot(diff,dt),fp=dot(dt,dt)+dot(diff,d2);
        if(abs(fp)>1e-10)t=clamp(t-f/fp,0.0,1.0);}
    return length(p-evalCubic(P0,P1,P2,P3,t));
}

// ── Winding number ──
constant int WK_MIN=2, WK_MAX=16; constant float WK_TH=0.01;
int adaptK(float2 A, float2 B, float2 C){return clamp(int(ceil(length(A-2.0*B+C)/WK_TH)),WK_MIN,WK_MAX);}
float waSeg(float2 p, float2 P1, float2 P2){float2 v1=P1-p,v2=P2-p;return atan2(cross2D(v1,v2),dot(v1,v2));}
float waBQ(float2 p, float2 A, float2 B, float2 C, int K){
    float tot=0; float2 prev=A;
    for(int k=1;k<=K;k++){float2 cur=evalQuad(A,B,C,float(k)/float(K));tot+=waSeg(p,prev,cur);prev=cur;}
    return tot;
}

float sdCSpline(float2 p2d, constant SDFNodeGPU* nd, int hIdx, int N, float th) {
    float2 pts[64]; int pi=0, ci=hIdx+1, mc=(N+2)/3;
    for(int c=0;c<mc&&pi<N;c++){SDFNodeGPU dc=nd[ci+c];
        if(pi<N)pts[pi++]=float2(dc.pos_x,dc.pos_y);
        if(pi<N)pts[pi++]=float2(dc.param_x,dc.param_y);
        if(pi<N)pts[pi++]=float2(dc.param_z,dc.param_w);}
    N=pi; if(N<2)return 1e10;
    float md=1e10;
    if(N==2){md=min(md,distQBez(p2d,pts[0],(pts[0]+pts[1])*0.5,pts[1]));}
    else if(N==3){md=min(md,distQBez(p2d,pts[0],pts[1],pts[2]));}
    else{md=min(md,distQBez(p2d,pts[0],pts[1],(pts[1]+pts[2])*0.5));
        for(int s=1;s<N-3;s++)md=min(md,distQBez(p2d,(pts[s]+pts[s+1])*0.5,pts[s+1],(pts[s+1]+pts[s+2])*0.5));
        md=min(md,distQBez(p2d,(pts[N-3]+pts[N-2])*0.5,pts[N-2],pts[N-1]));}
    if(th>1e-6)return md-th;
    // Winding
    float ta=0;
    if(N==2){float2 m=(pts[0]+pts[1])*0.5;ta+=waBQ(p2d,pts[0],m,pts[1],adaptK(pts[0],m,pts[1]));}
    else if(N==3){ta+=waBQ(p2d,pts[0],pts[1],pts[2],adaptK(pts[0],pts[1],pts[2]));}
    else{{float2 A=pts[0],B=pts[1],C=(pts[1]+pts[2])*0.5;ta+=waBQ(p2d,A,B,C,adaptK(A,B,C));}
        for(int s=1;s<N-3;s++){float2 A=(pts[s]+pts[s+1])*0.5,B=pts[s+1],C=(pts[s+1]+pts[s+2])*0.5;ta+=waBQ(p2d,A,B,C,adaptK(A,B,C));}
        {float2 A=(pts[N-3]+pts[N-2])*0.5,B=pts[N-2],C=pts[N-1];ta+=waBQ(p2d,A,B,C,adaptK(A,B,C));}}
    const float AXR=-1e-4;
    float2 lp=pts[N-1],fp=pts[0],ae=float2(AXR,lp.y),as=float2(AXR,fp.y);
    ta+=waSeg(p2d,lp,ae)+waSeg(p2d,ae,as)+waSeg(p2d,as,fp);
    return abs(ta)>3.14159265?-md:md;
}

// ── map() ──
float map(float3 pos, constant SDFNodeGPU* nd, int nc) {
    float stk[64]; int sp=0;
    for(int i=0;i<nc;i++){
        SDFNodeGPU n=nd[i]; if(n.type==C_DC)continue;
        float3 np=ndPos(n); float4 pm=ndPar(n);
        if(n.type==C_SPHERE){stk[sp++]=length(pos-np)-pm.x;}
        else if(n.type==C_BOX){float3 d=abs(pos-np)-pm.xyz;stk[sp++]=length(max(d,0.0))+min(max(d.x,max(d.y,d.z)),0.0);}
        else if(n.type==C_CYL){float3 lp=pos-np;float dR=sqrt(lp.x*lp.x+lp.z*lp.z)-pm.x;float dA=abs(lp.y)-pm.y;
            stk[sp++]=sqrt(max(dR,0.0)*max(dR,0.0)+max(dA,0.0)*max(dA,0.0))+min(max(dR,dA),0.0);}
        else if(n.type==C_TORUS){float3 lp=pos-np;float qr=sqrt(lp.x*lp.x+lp.z*lp.z)-pm.x;stk[sp++]=sqrt(qr*qr+lp.y*lp.y)-pm.y;}
        else if(n.type==C_CAPSULE){float3 A=np,B=pm.xyz;float rad=pm.w;float3 pa=pos-A,ba=B-A;
            float bb=dot(ba,ba);float t=bb>1e-10?clamp(dot(pa,ba)/bb,0.0,1.0):0.0;stk[sp++]=length(pa-ba*t)-rad;}
        else if(n.type==C_CIRCLE2D){float2 p2=float2(length(pos.xz),pos.y);stk[sp++]=length(p2-ndPos2(n))-pm.x;}
        else if(n.type==C_RECT2D){float2 p2=float2(length(pos.xz),pos.y);float2 d=abs(p2-ndPos2(n))-pm.xy;
            stk[sp++]=length(max(d,0.0))+min(max(d.x,d.y),0.0);}
        else if(n.type==C_BEZ2D){float2 p2=float2(length(pos.xz),pos.y);
            stk[sp++]=distQBez(p2,ndPos2(n),pm.xy,pm.zw)-n.pos_z;}
        else if(n.type==C_CBEZ2D){SDFNodeGPU ex=nd[i+1];float2 p2=float2(length(pos.xz),pos.y);
            stk[sp++]=distCBez(p2,ndPos2(n),pm.xy,pm.zw,float2(ex.pos_x,ex.pos_y))-n.pos_z;i++;}
        else if(n.type==C_CSPLINE2D){float2 p2=float2(length(pos.xz),pos.y);int N=int(pm.x);
            stk[sp++]=sdCSpline(p2,nd,i,N,pm.y);i+=(N+2)/3;}
        else if(n.type==C_UNION){float d2=stk[--sp],d1=stk[--sp];stk[sp++]=min(d1,d2);}
        else if(n.type==C_SUB){float d2=stk[--sp],d1=stk[--sp];stk[sp++]=max(d1,-d2);}
        else if(n.type==C_INTER){float d2=stk[--sp],d1=stk[--sp];stk[sp++]=max(d1,d2);}
        else if(n.type==C_SMOOTH){float d2=stk[--sp],d1=stk[--sp];float k=n.smoothFactor;
            if(k<1e-10){stk[sp++]=min(d1,d2);}else{
                float h=clamp(0.5+0.5*(d2-d1)/k,0.0,1.0);stk[sp++]=d2*(1.0-h)+d1*h-k*h*(1.0-h);}}
        else if(n.type==C_XFORM){float sf=pm.x;float cd=stk[--sp];stk[sp++]=cd*sf;i+=2;}
    }
    return sp>=1?stk[0]:1e10;
}

// ── Normal ──
float3 calcNormal(float3 p, float rt, constant SDFNodeGPU* nd, int nc, constant RenderParams& rp) {
    float eps=max(rp.minNormalEps,rt*rp.relativeNormalEps);float2 e=float2(eps,0);
    return normalize(float3(map(p+e.xyy,nd,nc)-map(p-e.xyy,nd,nc),
                            map(p+e.yxy,nd,nc)-map(p-e.yxy,nd,nc),
                            map(p+e.yyx,nd,nc)-map(p-e.yyx,nd,nc)));
}

// ── Vertex ──
vertex RasterizerData vertex_main(uint vid [[vertex_id]]) {
    RasterizerData out; float2 g[4]={float2(-1,-1),float2(1,-1),float2(-1,1),float2(1,1)};
    out.position=float4(g[vid],0,1); out.uv=g[vid]; return out;
}

// ── Fragment ──
fragment float4 fragment_main(
    RasterizerData in [[stage_in]],
    constant Uniforms& u [[buffer(1)]],
    constant SDFNodeGPU* sdf [[buffer(2)]],
    constant int& cnt [[buffer(3)]],
    constant RenderParams& rp [[buffer(4)]]
) {
    float3 ro=float3(u.camPosX,u.camPosY,u.camPosZ);
    float3 fwd=float3(u.camFwdX,u.camFwdY,u.camFwdZ);
    float3 rt=float3(u.camRightX,u.camRightY,u.camRightZ);
    float3 up=float3(u.camUpX,u.camUpY,u.camUpZ);
    float3 rd=normalize(in.uv.x*rt+in.uv.y*up+fwd);

    float bgMix=in.uv.y*0.5+0.5;
    float3 bgCol=mix(float3(rp.bgBottomR,rp.bgBottomG,rp.bgBottomB),
                     float3(rp.bgTopR,rp.bgTopG,rp.bgTopB),bgMix);

    float t=0; int steps=0;
    for(int i=0;i<rp.maxSteps;i++){
        float3 p=ro+rd*t; float d=map(p,sdf,cnt);
        float hitEps=max(rp.minHitEps,t*rp.relativeHitEps);
        if(d<hitEps){
            float3 n=calcNormal(p,t,sdf,cnt,rp);
            float3 ld=normalize(rt+2.0*up+fwd);
            float3 vd=-rd;float3 ref=reflect(-ld,n);
            float diff=max(dot(n,ld),0.0);
            float spec=pow(max(dot(ref,vd),0.0),rp.specularPower);
            float ao=1.0-float(steps)/float(rp.maxSteps)*0.5;
            float3 bc=float3(rp.baseColorR,rp.baseColorG,rp.baseColorB);
            float3 col=bc*(rp.ambient+rp.diffuseStrength*diff)*ao+rp.specularStrength*spec;
            return float4(col,1.0);
        }
        if(t>rp.maxDistance)break;
        t+=d*rp.stepSafetyFactor; steps++;
    }
    return float4(bgCol,1.0);
}
